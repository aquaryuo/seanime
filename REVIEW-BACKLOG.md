...
**Fix** Widen the fallback reaper to match the profile-dir marker (`-f 'user-data-dir=.*aquatils'`), which covers a system browser too. Do **not** pkill the display server by name — that needs a marker the solver emits, which lands in the solver repo along with graceful-shutdown handling.
**Effort** hours · **Risk** medium
...
**What** `setErr` is the only caller of `aqReport` and hard-codes the scope. So download failures, extraction failures, launch exceptions and release-feed errors all publish as `[aq-aquatils · solver]`, while errors from the solver *process* never enter the channel at all.
...
Thirteen bare identifiers (`"fetch"`, `"shape"`, `"attr"`, …) are forwarded into the error pane as `[aq-seatags · decorate] fetch`. **Fix**: keep the code as the dedup key, pass a sentence as the message. Delete the write-only `dErr` field.
**Effort** minutes · **Risk** low
...
| `animepahe-playpage-predicate` | `looksLikePlayPage` accepted any body containing `data-src`, which the *series* page is full of, so a wrong page was accepted, cached for five minutes and then parsed to zero sources. The parse is now the validator (`parsePlaySources(b, audio).length > 0`) and the cache holds the parsed `PlaySource[]` rather than 30-50 KB of HTML. The cache key had to move to `apahe:play2:` **and gain the audio**: a stale entry from the old key is a *string*, and `readCache<PlaySource[]>(…).length > 0` would have been true for it — a silent type confusion |
| `animepahe-error-protocol` | the search loop branched on six English substrings from messages written elsewhere in the file, one of which (`"blocked"`) already matched nothing. `fail()` now records a `lastFailKind` and the loop tests that; `lastFailKind` and `lastResp` are cleared at the top of each iteration |
...
| `seatags-beta-invisible` | `c13b934` — this repo's own `marketplace.json` is merged in as a fallback source under the community list (community wins on id collision), so the beta extensions that are absent upstream resolve an author instead of being hidden by both "Working" and author search. Same host as the existing source, so no permission change; the URL is channel-relative and the promotion rewrite handles stable |
| `anikoto-double-search` | `d946b48` — the host calls `Search()` twice per episode-list load (romaji, then english) and `searchQueries` produces the same set both times, so the outcome is cached for 60 s. The key is the **sorted** query set — the two calls build the same set in different order, so an unsorted key would never hit — and it is keyed on the queries, not `media.id`, because manual search passes no Media and every such call would collide on id 0 |
| `aquaprefs-scope` | `2651a5d` — the write-only `pref:__index` is gone, the key is one named constant, and the manifest now says what it does ("one set of player options, reapplied on every video") instead of promising per-series memory. The index is read once, behind a flag, to purge the orphaned `pref:m:`/`pref:e:` keys before it is dropped |
| `aquaprefs-track-match-key` (provider half) | all three providers already emit `"<code>-<idx>"` ids, so the `VideoSubtitle.id` contract the entry asks for now holds. The aquaprefs matching half is still open |
| `anikoto-meta-remap-numbering` | `a6f4fa0` — the numbering choice is made once by set overlap (`ep` vs `abs` hits against the site's own numbers, ties to `ep`) instead of guessing per key from a length comparison; `map[k]` is guarded, `perPart` is gone, and a discarded remap now says so instead of failing silently. The `/meta` catch that swallowed everything reports too |
| Nits (Batch H) | `7d0c835` — the four providers declare `implements AnimeProvider`; anikoto's fallback episode number counts what it emitted rather than the raw node index; animepahe's `getText` keeps the exception so the diagnostic is not blank in the one case it helps; `unpack`'s symbol lookup is bounded by the packer's own count |
| `animepahe-season-filter`, `animepahe-mirror-select-inert` | `ed2d669` — the search pool is narrowed to the same show before season and part are applied as independent predicates, and the hermetic suite proves it: against the previous build the *only* result for "Attack on Titan Season 2" was an unrelated show's season 2. The mirror select keeps its three options but says which two redirect, and `resolveBase` no longer probes the two that land on the third. animelok's `normBase()` is hoisted to a field — same one-time legacy remap, off the hot path |
| `aquaprefs-caption-event-dead`, `aquaprefs-caption-index`, `aquaprefs-vs-native-preference` | the whole `cap` domain is gone — the host never emits `video-media-caption-track`, so every load fired one blind `setMediaCaptionTrack`. Enforcement now also stands down entirely when `onlinestreamParams` is present: Seanime remembers that per series and aquaprefs was overwriting it globally. What is left is local and MKV playback, where nothing else remembers anything, and the manifest and the toggle say so. This also retires `aquaprefs-track-match-key`'s plugin half — the label-vs-language key it describes only ever collided on online streams |
| Nits (Batch H, anizone) | one attribute tokenizer replaces `tagAttr` and the three heuristics leaning on it. **The "the site's escaping makes it inert" note in that nit was wrong** — proved against the previous build: for `<track label="x src=&quot;wrong&quot;" src=right.vtt>` the old reader returns `&quot;wrong&quot;"`, because the escaping defeats the two quoted patterns and the *unquoted* pattern then matches the escaped text inside the label. `<track src=a.vtt/>` likewise returned `a.vtt/` with the tag's own closing slash attached. Both are 404 subtitles, not visible errors. The tokenizer also decodes every value rather than only the label, and `default` is now read as an attribute instead of matched anywhere in the tag. Plus: `decodeEntities` handles astral codepoints and leaves out-of-range ones alone; `langName` gains 36 codes; the legacy card parser no longer requires a lowercase port-less origin and pairs blocks to hrefs in one pass instead of a nested scan |
| `seatags-author-filter-hides-everything` (structural half), `seatags-installed-id-fallback` | the author is read off the card's own badge, so author search works for every installed extension whether or not the remote list knows it, and the cards observer arms without waiting for that list. The badge is identified by class tokens rather than position: `rounded-md` with neither `tracking-wide` nor `border-transparent` uniquely picks the author out of Built-in / Disabled / version / lang, verified against upstream `extension-card.tsx` rather than guessed. A name+author map now disambiguates the duplicate names that `byName` used to drop entirely. **Found while reading that component: `extractId` does not extract an id.** The card renders the *description* in the `opacity-30` element and never renders `extension.id` as text, so the `byId` path has always been dead and every card has resolved by non-unique name. It is left in place rather than removed — it costs nothing if some Seanime version does render an id there — but the name+author path is what actually matches now |
| User report: anikoto subtitles 500 on nearly every episode, video fine | **root cause is outside the code: the operator's ISP filter is DNS-sinkholing the subtitle CDN.** megaplay serves subtitles from a *different* host than the video — here `sqitq.velvetcomet.top` vs `fetch.nexabloom.top`. That subtitle host resolves to a sinkhole presenting a certificate issued by `CN=Whalebone Sinkhole CA`, and an insecure fetch 307-redirects to `redirect.a1-onlineschutz/passthrough?...` carrying `"SinkholeID":2093` — A1's Onlineschutz parental filter. Every TLS-verifying client (Go, Node, curl) refuses the chain, so Seanime's `FetchAndConvertSubsTo` fails and `POST /api/v1/directstream/subs/convert-subs` returns 500; confirmed in Seanime's own log. The video is untouched because its host carries a valid Let's Encrypt chain. Not a regression: the commit before this touched no subtitle code. **Mitigated anyway, because the same subtitle path is served byte-for-byte by the video's host**: `alignSubtitleHost` probes the default track and, only if it is unreachable *and* the video host can serve it, rewrites every subtitle onto the host the video already plays from, reporting the swap once. It costs zero requests when both already share a host (the common case) and at most two otherwise, and is skipped when out of budget. Proven end to end through the real production path — sinkholed host `convert-subs` 500, swapped host 200 with 27158 bytes of ASS |
| User report: anikoto "Loading stream" on one episode, self-healing after ~5 min | **investigated with 113 agents; 34 of 35 candidate findings were refuted, and the survivors were verified live.** Three live facts drive it. (1) **The advertised CDN is dead.** megaplay hands out `fetch.nexabloom.top`, which returns 403 for every server, both audio tracks, every episode and multiple series — confirmed through *Seanime's own `/api/v1/proxy`*, single request, after a 200 s cooldown, so it is neither rate-limiting nor a client artifact. anikoto plays at all today only because `playableOnKnownHost` swaps onto the hardcoded `ncdn.imgnex.top`, which returns a real `#EXTM3U` through the same proxy. (2) **That rescue was gated behind the budget**: `if (h === current || this.outOfTime()) continue` skipped every host once the 75 s budget was spent — the one path that works was the first thing dropped under load. (3) **The budget was being spent on nothing**: megaplay never returns `sources`, only `enc` (payload keys `tracks,t,intro,outro,server,enc`), so `discoverSourcePaths` ran on *every* resolve, and `encKeys` awaited a full script scan *before* trying the hardcoded key one line above it — and all three "servers" (`HD-1`/`HD-2`/`Vidstream-2`) decode to one byte-identical URL, so that waste was paid three times. Fixed: known keys tried before any scan; enc decoded before hunting a moved endpoint; the host swap always gets one attempt (bounded to one request past the deadline rather than the six the analysis proposed); aliased candidates skipped; entries proven unplayable evicted instead of pinned for 5 min; and the two "endpoint moved"/"cipher changed" alarms now fire only on something genuinely new instead of on every single resolve. A hermetic test pins the cost — it fails on the old code (embed page fetched 3x) and passes now (1x, 6 requests down to 4). **Honest gap: why episode 3 and not episode 4 is still unexplained** — nothing in the provider is episode-dependent, so the trigger was an upstream transient; these defects are what turned a transient into a dead end. The ~5-minute recovery matches `serverCacheTtl` and nothing else the user could not clear, but the resolved URL was byte-stable across 200 s, so expiry explains why every retry was identical, not provably why the outcome flipped |
| User report: anizone episode list takes 30-40 s | **cause found by measurement, one-word fix.** Not the pagination budget, which never even runs — One Piece's series page carries no `gotoPage(` markers at all, so the page walk is dead for it. The whole cost was `trimToExisting`, the tail probe that checks whether the site's stated count is real, and it used `method: "HEAD"`. **anizone never completes a HEAD**: `curl -I` returns the status line and then hangs until the client gives up (40 s, zero bytes). Under Go's 35 s default fetch timeout one probe alone eats the whole budget, which is exactly the reported 30-40 s. The same URLs by GET answer in ~0.25 s for a 404 and ~0.5 s for a 200. Measured end to end against the live site: One Piece `findEpisodes` **27.8 s → 1.2 s warm**, same 1177 episodes; a 28-episode series needs one probe. The test suite now asserts the probe never uses a method other than GET |
| User report: anikoto subtitles all 500 | **regression I shipped, now removed.** Replacing `sub.ryuo.to` I had anikoto fetch each track server-side and inline it as a `data:text/vtt;base64,…` URI, on the belief that the host drops subtitle headers and passes URLs to the client raw. That belief was wrong. `onlinestream-page.tsx` computes `shouldProxy = !!headers && Object.keys(headers).length > 0` and, when true, rewrites **both** the video URL and every subtitle URL to `/api/v1/proxy?url=…&headers=…`. anikoto always sets `Referer`/`Origin`, so every subtitle was handed as a `data:` URI to a server-side HTTP fetcher — hence axios 500 on all of them, every time. The referer gating that motivated the inlining was already solved by that same proxy, so the fix is to return the raw CDN URL and let the host send the headers. `inlineTrack`, `asDataUri` and the inline budget are gone and `buildSubtitles` no longer needs to be async. **The stored note for this was self-contradictory** — one bullet said headers never reach subtitles, a later one in the same file said they do and correctly described the proxy; I acted on the wrong one. Corrected, and the two halves are no longer in separate bullets |
| `anizone-dub-selects-nothing` | **resolved by upstream, verified in its source, not worked around here.** The entry's "durable fix is upstream" now exists: `onlinestream.atoms.ts` defines `__onlinestream_audioTrackPreferenceByMediaAtom` and `onlinestream-page.tsx` reads the HLS audio tracks, offers a picker, matches a remembered choice by language then name then id, and reapplies it per media. So the stream is right, the control exists, and the choice sticks after one use — the only residue is that the *first* play follows the master's `DEFAULT=YES`, which no provider can override. Neither suggested workaround was taken: a duplicate `videoSource` would exist purely to trip a host UI path and would put two identical entries in the quality picker. The gate stays as it is (refuse when the master carries no English rendition) and the manifest now says where the audio switch lives. Same visit confirmed `__onlinestream_subtitlePreferenceByMediaAtom` exists, which is what today's `aquaprefs-vs-native-preference` change stands down for |
| `config-untested` | 25 checks that vary configuration, which `userconfig-placeholders` made possible: each provider's fallback to its manifest default, rejection of an unsubstituted `{{placeholder}}` and of an empty setting, a non-default mirror actually taking effect, animelok's legacy-domain remap, and the solver's on/off, endpoint normalisation and malformed-URL paths. Writing them found one thing: animepahe's "Solver endpoint not set" fires for a *malformed* endpoint, never an unset one — `cfg()` substitutes the default for an empty field, so the only way to reach that branch is to type something that is not an http(s) URL. Message corrected to say that. It also settled that clearing the solver URL does not disable the solver; `useCustomSolver` is the switch |
| CI gap: stray control bytes | a NUL byte landed in a payload during this session's edits and **passed tsc, esbuild and the whole suite** — it is a legal string character. The pure suite now scans all seven payloads for any control byte outside tab/CR/LF, and the check was proved by planting one. Fourth green-but-wrong gate found in this project |
| Nit (Batch A, anikoto) | when a whole search page carries cards but zero `.ep-status` badges, anikoto says so once per search instead of silently calling every result sub-only. That case is worse than a wrong label for a dub search — `if (dub && !hasDub) return` drops every card, so a badge markup change reads as "no dub exists" for the entire site |
| `fail-contract-wrapper`, `anikoto-ajax-unguarded`, `anikoto-serverlist-bare-fetch` | a `guarded(scope, url, opts)` entry point in both providers. A transport failure now records SEHERRv1 with the detail, invalidates the mirror (anikoto), and throws one plain sentence instead of letting a raw Go error carry the full request URL into the player. Four unguarded sites are gone — anikoto's `/ajax/episode/list` and `/ajax/server/list`, anizone's series and episode pages — and the two anikoto AJAX endpoints gained the challenge check they never had. Every `fetch` left in either class sits inside its own `try`. The two AJAX bodies are read once and parsed from that text rather than calling `.text()` and `.json()` on the same response |
| `fail-vs-report`, `animelok-anilist-only-search` | anikoto's three speculative throws inside `resolveServer` no longer report — the Auto loop absorbs them, so a user whose episode played on the second candidate was being shown the first candidate's failure as an error. They carry the server name now and the loop keeps the last one, which the single terminal `fail()` folds in; a specific reason also replaces the generic "enable the solver" hint rather than being appended to it. animelok says outright that it matches by AniList id when handed a title, instead of returning nothing and advising manual matching that cannot work — an empty query still returns nothing quietly |
| Nits (Batch B, anikoto) | the audio guard is symmetric now — a sub request that resolves to a `/dub/` embed is refused the same way the reverse already was. Neither side fires on an unrecognised URL, so hardsub (`/hsub/`, which the pattern deliberately misses) and unknown shapes still pass. The solver POST carries `noCloudflareBypass: true`, matching what animepahe sets on the identical loopback endpoint. The "manual path does not probe playability" half is retired rather than fixed: `server-arg-contract` collapsed both paths into one Auto loop that probes every candidate |
| `payload-sha-pin` | **not taken — the shape does not close.** A manifest cannot carry the SHA of the commit that contains it, so pinning needs a second commit that rewrites `payloadURI` to the first one's SHA, which splits every payload change across two commits and puts the `version-bump-gate` job in the position of judging a commit that changes a payload with no sibling manifest. Blob SHAs would sidestep the ordering, but `raw.githubusercontent.com` only serves commit-ish refs and the API returns base64 JSON, which is not what the host fetches. Weighed against a Sev-low race inside a 5-minute cache window, a wrong SHA is a payload that 404s permanently. Revisit only if the release path ever becomes a tag |
| `semver-constraint` | **dismissed, premise does not hold.** All eight `$scannerUtils` call sites across the three providers are inside `try`/`catch`, so an older host degrades to worse matching rather than failing. `">=3.5.1"` would be stricter than the code requires and, since enforcement is all-or-nothing, would invalidate extensions that work today. The entry already ruled out guessing for the two plugins, so nothing is left to add |
| `seatags-status-dropdown-a11y` | a real `<select>` with a `change` listener and an `aria-label`, plus a label on the author input. It deletes the trigger/menu/check machinery, the hover stylesheet and the body-click listener that leaked with it — 97 lines. The cost is the pixel-match with the host's own Select |
| `seatags-version-overwrite`, `seatags-hidden-badges`, `seatags-row-never-restored` | `78d35fb` — one change: append the chips, never hide the native row. The remote list no longer overwrites anything describing the local install — version, author and language chips are gone, leaving only the status tags and stars, which are genuinely community data. Built-in / Disabled / the clickable version-diff / "Update available" all survive, and a card that stops matching keeps its native badges |
