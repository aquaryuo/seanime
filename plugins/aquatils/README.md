...
On Linux, Stage B needs Xvfb and Chromium's system libraries. The plugin never installs packages itself: when something is missing, the tray shows a command to copy — apt, dnf or zypper built from the downloaded Chromium's own dependency list, fixed package names for pacman and apk, a configuration snippet on NixOS; on other systems it names what is missing. Run it in a terminal, then press Start. On musl systems (Alpine) and NixOS, where Google's Chromium build can't run, the plugin uses a Chromium-based browser already installed on the system (`chromium`, Chrome, Brave or Edge) instead of downloading one (snap wrappers are skipped).
...
- `commandScopes` `sh` / `cmd` — start the downloaded solver and prepare the downloaded Chromium (`chmod` and the macOS quarantine strip on both; on Windows `.\solver.exe` from its own folder); hash the download before it runs (`sha256sum` / `shasum` / `certutil`); unpack Chromium on macOS (`ditto`); look for an installed Chromium-based browser (`command -v`); on Linux, check read-only what the browser needs (`command -v` for `Xvfb` and the package manager; `readlink` / `grep` on that browser's launcher to skip snap wrappers; `ldd` on the downloaded Chromium; `/etc/os-release` for NixOS and `ldd --version` for musl) so the tray can show the install command you run yourself — nothing is installed by the plugin; stop the plugin's own solver and Chromium processes (`pkill` / `kill`, PowerShell), matched exactly on this plugin's own cache folder (a program inside it, or a browser whose profile is inside it), and wait for the port to close (`ss` / `lsof`). Decline if Remote-only.
